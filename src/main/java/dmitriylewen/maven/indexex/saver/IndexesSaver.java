        ...
        for (int i = lastIndex; ; i--) { // parse all archives from lastIndex to last existing one
            try (UrlResource urlResource = new UrlResource(new URL(String.format(MAVEN_REPOSITORY_BASE_URL + ARCHIVE_NAME, i)));
                ...
                try {
                    ...
                    System.out.printf("error: can't save indexes to json: %s\n", e);
                }
                ...
                System.out.println("Parse of archives finished.");
                System.out.printf("last archive: " + ARCHIVE_NAME + "\n", i - 1);
                System.out.printf("Saved %d indexes\n", numberOfIndexes);
                break;
            } catch (IOException e) {
